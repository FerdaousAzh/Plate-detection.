...
class PlateDetector:
    ...
    def draw_labels(self, boxes, confidences, class_ids, img):
        ...
        return img,plats
    def retrieve_plates_subimages_from(self, img_path, weight_path, cfg_path):
        self.load_model(weight_path, cfg_path)
        img, height, width, channels = self.load_image(img_path)
        blob, outputs = self.detect_plates(img)
        boxes, confidences, class_ids = self.get_boxes(outputs, width, height)
        _, plates = self.draw_labels(boxes, confidences, class_ids, img)
        return plates
    def retrieve_plates_subimages_from_img(self, img, weight_path, cfg_path):
        self.load_model(weight_path, cfg_path)
        height, width, channels = img.shape
        blob, outputs = self.detect_plates(img)
        boxes, confidences, class_ids = self.get_boxes(outputs, width, height)
